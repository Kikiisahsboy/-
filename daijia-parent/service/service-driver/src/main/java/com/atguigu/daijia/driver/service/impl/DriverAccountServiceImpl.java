...
public class DriverAccountServiceImpl extends ServiceImpl<DriverAccountMapper, DriverAccount> implements DriverAccountService {
    ...
    public Boolean transfer(TransferForm transferForm) {
        ...
        if (count > 0) {//该订单已将钱转给司机
            return true;
        }

        //2 添加奖励到司机账户表
        LambdaQueryWrapper<DriverAccount> driverAccountWrapper = new LambdaQueryWrapper<>();
        driverAccountWrapper.eq(DriverAccount::getDriverId, transferForm.getDriverId());
        DriverAccount driverAccount = driverAccountMapper.selectOne(driverAccountWrapper);
        driverAccount.setTotalAmount(driverAccount.getTotalAmount().add(transferForm.getAmount()));
        driverAccount.setAvailableAmount(driverAccount.getAvailableAmount().add(transferForm.getAmount()));
        driverAccount.setTotalIncomeAmount(driverAccount.getTotalIncomeAmount().add(transferForm.getAmount()));
